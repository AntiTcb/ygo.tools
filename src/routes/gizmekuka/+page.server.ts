...
export const load: PageServerLoad = async ({ locals }) => {
  const [cardsRes, monsterTypesRes, cardFrameTypesRes] = await Promise.all([
    locals.supabase
      ...
      .select(CARD_SELECT)
      .eq('language', 'en')
      .not('frame_type_id', 'in', `(${GIZMEK_UKA_EXCLUDED_FRAME_TYPE_IDS.join(',')})`),
  ...
  if (cardsRes.error) {
    console.error('error loading gizmek uka monsters', cardsRes.error);
  ...
  return {
    cards: (cardsRes.data ?? []).filter((card) => isLegalGizmekUkaMonster(card, tokenFrameTypeIds)),
    lookups: {
