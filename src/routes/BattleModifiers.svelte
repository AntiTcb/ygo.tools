<script lang="ts">
    ...
    let {
        modifiers = $bindable<BattleModifiers>(),
        showExamples = $bindable<boolean>(false),
    ...
    {#if showExamples}
        <div class="flex flex-wrap gap-1 pt-1 text-xs italic">
            ...
            {#each cards.sort((a, b) => a.name.localeCompare(b.name)) as { name, id } (id)}
                <a href="https://www.db.yugioh-card.com/yugiohdb/card_search.action?ope=2&cid={id}" target="_blank" class="chip preset-filled"
                    >{name}</a>
            {/each}
        </div>
    {/if}
